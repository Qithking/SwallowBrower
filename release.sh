...
REPO_NAME=$(get_remote_info || echo "SwallowBrower")
...
# 获取最新版本号
get_latest_release() {
    curl -s "https://api.github.com/repos/Qithking/SwallowBrower/releases/latest" 2>/dev/null | \
...
create_release() {
    ...
    commit_count=$(gh api repos/Qithking/SwallowBrower/commits --jq '. | length' 2>/dev/null || echo "0")
    if [ "$commit_count" = "0" ]; then
        # 备用方案：获取总提交数
        commit_count=$(gh api repos/Qithking/SwallowBrower/commits --paginate --jq 'length' 2>/dev/null || git rev-list --count HEAD 2>/dev/null || echo "0")
    fi
    ...
    echo_info "查看构建进度: https://github.com/Qithking/SwallowBrower/actions"
}
...
download_latest() {
    ...
    if command -v gh &> /dev/null && gh auth status &> /dev/null; then
        latest_info=$(gh release view --repo Qithking/SwallowBrower --json tagName,url 2>/dev/null || echo "")
        latest_version=$(echo "$latest_info" | grep -o '"tagName"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
    fi
    
    # 备用方案: 使用 API
    if [ -z "$latest_version" ]; then
        latest_version=$(curl -s "https://api.github.com/repos/Qithking/SwallowBrower/releases/latest" 2>/dev/null | \
            grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/' || echo "")
    fi
    ...
    # 下载 DMG
    dmg_name="SwallowBrower-${latest_version}-universal.dmg"
    download_url=""
    
    # 尝试从 GitHub Release 获取下载链接
    if command -v gh &> /dev/null && gh auth status &> /dev/null; then
        download_url=$(gh release view "$latest_version" --repo Qithking/SwallowBrower --json assets -q '.assets[] | select(.name | contains("dmg")) | .url' 2>/dev/null || echo "")
    fi
    
    # 构建直接下载链接
    if [ -z "$download_url" ]; then
        download_url="https://github.com/Qithking/SwallowBrower/releases/download/${latest_version}/${dmg_name}"
    fi
...
clean_failed_actions() {
    ...
    # 获取失败的运行
    failed_runs=$(gh run list --repo Qithking/SwallowBrower --status failure --json databaseId,workflowName --jq '.[] | "\(.databaseId) \(.workflowName)"' 2>/dev/null)
    ...
    if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
        ...
        echo_info "正在删除..."
        echo "$failed_runs" | while read -r id name; do
            gh run delete "$id" --repo Qithking/SwallowBrower 2>/dev/null && echo_success "已删除: $name ($id)" || echo_error "删除失败: $id"
        done
        echo_success "清理完成!"
    else
        echo_info "已取消"
